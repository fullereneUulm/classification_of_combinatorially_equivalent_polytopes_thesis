...
k_star=np.loadtxt("k_star_L6_single", delimiter = "  ", dtype = int)

for n in range(26,100,2):
    if sum(k_star[:,0]==n)==0:
        col=int(n/2-10)
        f = int(n/2+2)
        ...
        #If no such row exists, set it -1   
        if k_star_n==0:
            k_star_n=-1
...
#Note that the table is saved transposed, because of legibility
np.savetxt("k_star_L6_single",k_star, fmt="%i", delimiter= "  ")
print("Done")
